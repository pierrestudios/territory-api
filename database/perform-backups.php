...
function performDBBackup()
{
    ...
    if ($return_var === 0) {
        ...
        mail_attachment(
            $filename, 
            $path, 
            $mailto = 'webdevsolutionsstudio@gmail.com', 
            $from_mail = 'mailer@webdevstudio.net', 
            $from_name = 'Territory Api', 
            $replyto = 'mailer@webdevstudio.net', 
            $subject = 'Territory Api Database Backup', 
            $message = 'Database backup for '. date('m-d-Y', time())
        );
    } else {
        echo 'failed: ';
        var_dump($output);

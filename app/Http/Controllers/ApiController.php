...
class ApiController extends BaseController {
	...
	public function signup(Request $request) {
		...
		if (!empty($userExist)) return Response()->json(['error' => 'User with that email already is in the system', 'message' => 'User with that email already is in the system'], 500);
	...
	public function authUser(Request $request) {
		...
		// $authUserFromToken = JWTAuth::toUser($this->parseAuthHeader($request));
	...
	*/
	protected function hasAccess($request) {
		...
		// if (empty(Auth::user())) 
		Auth::loginUsingId($user->id); // simulate user login
	...
	*/
	protected function unTransformCollection($collection, $type) {
		...
		foreach ($collection as $i => $entity) {
			if (gettype($entity) != 'array' && gettype($entity) == 'object') $entity = (array)$entity;
	...
	protected function unTransform($data, $type) {
		...
		if ($type == 'address') {
			...
			foreach (Address::$transformationData as $k => $v) {
				...
				if (array_key_exists($k, $data) && $v == 'inactive' && ($data[$k] === '0' || $data[$k] === null || $data[$k] === false)) $transformedData[$v] = 0;

				// Set default "lat" property/field for address to 0.0
				if ($v == 'lat' && !array_key_exists($k, $data)) $transformedData[$v] = 0.0;
		...
		if ($type == 'note') {
			foreach (Note::$transformationData as $k => $v) {
				if (!empty($data[$k])) $transformedData[$v] = $data[$k];
				// Add default retain 
				// if ($k == 'retain' && empty($data['retain'])) $transformedData['archived'] = 0;
			}
			// Add default entity
	...
	*/
	protected function sortArrayByObjKeys($data, $keys = []) {
		...
		foreach ($data as $k => $address) {
			if (empty($sortedStreets[$address['streetId']])) $sortedStreets[$address['streetId']] = [];

(function () {
    ...
    angular.module('app')
            ...
            function ($rootScope, $scope, settings, $location, $localStorage, Auth) {
                ...
                $scope.signin = function () {
                    ...
                    Auth.signin(formData, successAuth, function (res) {
                        $rootScope.error = res.message || 'Invalid Infomation.';
                    })
                };
                ...
                $scope.signup = function () {
	                ...
	                if ($scope.password != $scope.passwordComfirm ) {
		                $rootScope.error = 'Password does not match.';
		                return;
	                }
        	...
        	function ($scope, $uibModalInstance, API, entity, Notify) {
			    ...
			    $scope.removeAddress = function () {
				    ...
				    if(!$scope.entity.note) {
					    Notify.error('Mete rezon pou w retiré adrès lan.');
					    return false;
				    }
        ...
        .controller('ApiController', ['$rootScope', '$scope', '$route', 'settings', 'API', '$location', '$localStorage', '$routeParams', '$uibModal', 'Notify',
        	function ($rootScope, $scope, $route, settings, API, $location, $localStorage, $routeParams, $uibModal, Notify) {
	            ...
	            $scope.MorrisData = function(Morris) {
	                ...
	                Morris.Area({
				        ...
				        labels: ['Territwa', 'Rapò', 'Pwoklamatè'],
				        xLabels: 'month',
				        pointSize: 2,
				        hideHover: 'auto',
				        resize: true
								...
								$('#dataTables-users').DataTable({
								    ...
								    "columnDefs": [{
							            ...
							            "render": function(data, type, fullObj, meta ) {
									        return data ? '<a href="#/publishers/'+ data.publisherId +'"><strong>' + data.firstName + ' ' + data.lastName + '</strong> <i class="fa fa-arrow-circle-right"></i></a>' : '<a href="" class="btn btn-info attach-publisher" data-user-id="'+ fullObj.userId +'">Ataché Pwoklamatè</a>';
									    }
							        },{
							            "targets": 3,
							            "data": "publisher",
							            ...
							            "render": function(data, type, fullObj) {
									        return '<a class="btn btn-info btn-sm edit-user" href="" data-user-id="' + data + '" data-user-email="'+ fullObj.email +'" data-user-type="'+ fullObj.userType +'">' + 'Changé' + '</a> ' + (window.isAdmin ? ' <a class="btn btn-sm btn-danger delete-user" data-user-id="' + data + '" data-user-email="'+ fullObj.email +'">' + 'Retiré' + '</a>' : '');
									    }
									}],
						            searching: false,
						            paging: false,
							...
							if(!$('#dataTables-publishers').is('.dataTable') && res.data && res.data.length) {
								$('#dataTables-publishers').DataTable({
								    ...
								    "columnDefs": [{
							            ...
							            "data": "publisherId",
							            "render": function(data, type, fullObj, meta ) {
									        return '<a class="btn btn-info btn-sm" href=#/publishers/' + data + '>' + 'Modifye' + '</a> ' + (window.isAdmin ? '<a class="btn btn-sm btn-danger delete-publisher" data-publisher-id="' + data + '" data-publisher-name="' + fullObj.firstName + ' '+ fullObj.lastName + '">' + 'Retiré' + '</a>' : '');
									    }
									}],
						            searching: false,
						            paging: false,
								...
								if(!$('#dataTables-addresses').is('.dataTable') && res.data.addresses && res.data.addresses.length) {	
									var table = $('#dataTables-addresses').DataTable({
										...
										},
									    {
								            ...
								            "render": function(data, type, fullObj) {
									            var isOwner = false;
									            var notes = (window.isEditor ? '<li class="list-group-item"><button class="btn btn-success btn-sm badge badge-success add-note" data-address-id="' + fullObj.addressId + '">' + 'Meté Nòt' + '</button> &nbsp; </li>' : '');
									            for(var n in data) {
										            isOwner = (data[n].userId == window.userId);
										            if(n < 5) notes += '<li class="list-group-item">'+ data[n].note + (data[n].date != '0000-00-00' ? ' <small class="label label-default">'+ data[n].date  +'</small>' : '') + ( isOwner ? ' <button class="btn btn-info btn-sm edit-note" data-note-id="' + data[n].noteId + '" data-note-note="' + data[n].note + '" data-note-date="' + data[n].date + '">Changé Nòt lan</button></li>' : ''); 
									            }
								            ...
								            "render": function(data, type, fullObj, meta ) {
									            ...
									            if (fullObj.inActive && window.isAdmin) {
										            isInActive = '<button class="btn btn-sm btn-default edit-address" data-address-id="' + data + '" data-address-address="'+ fullObj.address +'" title="Fè li actif">' + '<i class="fa fa-eye"></i>' + '</button>';
									            }
										        return (window.isEditor ? ' <button class="btn btn-info btn-sm edit-address" title="Modifye adrès lan" data-address-id="' + data.addressId + '"><i class="fa fa-edit"></i></button> ' + (isInActive ? isInActive : '<button class="btn btn-sm btn-danger delete-address" data-address-id="' + data + '" data-address-address="'+ fullObj.address +'" title="Retiré adrès lan">' + '<i class="fa fa-times"></i>' + '</button>') : '');
										    }
										}],
										"order": [[ 1, 'asc' ]],
							            searching: false,
							...
							if(!$('#dataTables-territories').is('.dataTable') && res.data && res.data.length) {
								$('#dataTables-territories').DataTable({
								    ...
								    "columnDefs": [{
							            ...
							            "data": "territoryId",
							            "render": function(data, type, fullObj, meta ) {
									        return '<a class="btn btn-info btn-sm" href=#/territories/' + data + '>' + 'Detay' + '</a>';
									    }
									}],
						            searching: false,
						            paging: false,
...
/*
							API.getTerritoryActivities($routeParams.territoryId, function(res) {
								if(res.data) {
...
*/
							            ...
							            timeline = new links.Timeline(document.getElementById('territory-activities'), options);
/*
							            	links.events.addListener(timeline, 'select', function(e) {
